...
public class ArtxeSQL {
    ...
    public boolean connect(){
        if(!isConnected()){
            ...
            plugin.getLogger().log(Level.INFO, MessageFormat.format("Host: {0}:{1}", this.host, this.port));
            plugin.getLogger().log(Level.INFO, MessageFormat.format("Database: ''{0}''", this.database));
            plugin.getLogger().log(Level.INFO, MessageFormat.format("Username: ''{0}''", this.username));
            plugin.getLogger().log(Level.INFO, MessageFormat.format("Using password: {0}", this.password.isEmpty() ? "NO" : "YES"));
